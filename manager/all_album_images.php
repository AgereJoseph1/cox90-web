...
$toast = "image";
	...
	if(isset($_GET['id']) && !empty($_GET['id'])) {
		...
		$albumr = mysqli_query($con, "SELECT name, description FROM albums WHERE album_ID = $album_id");
		$albumopened = mysqli_fetch_array($albumr);
		$sub_site_name = "View ".$albumopened['name']." Album";
						...
						<div class="col-lg-12">
							<div class="card-box">
								<div class="card-head p-2">
									<?php echo($albumopened['description']); ?>
								</div>
								<div class="card-body">
					<?php 
										
						$x =0;
						$images = get_image($album_id, $con);
						if(mysqli_num_rows(get_image($album_id, $con)) == 0) {
							echo('<h3 class="p-5">There are no images in '.$albumopened['name'].' album <a href="add_image.php">Upload images</a></h3>');
							...
							while ($images_row = mysqli_fetch_assoc($images)) { ?>
			                            ...
			                            <div class="col-lg-2 col-md-2 col-sm-6 m-b-20"  id="remove_<?php echo($images_row['ID']) ?>"> 
		                            	<div>
			                            <a href="../uploads/images/gallery/<?php echo($images_row['img_album_id']) ?>/<?php echo($images_row['img_name']) ?>" data-sub-html="<?php echo($albumopened['name']); ?> ALBUM"> 
			                            <img class="img-fluid img-thumbnail" src="../uploads/images/gallery/thumb/<?php echo($images_row['img_album_id']) ?>/resized_<?php echo($images_row['img_name']) ?>" alt=""> 
			                            </a>
							    		[<span href="" id="del_<?php echo($toast); ?>_<?php echo($images_row['ID']); ?>" class="fa fa-trash-o delete_<?php echo($toast); ?>"></span>]
		                            	</div>
			                            </div>
					<?php }
						}
