...
include('../functions/functions.php');
                ...
                if(isset($_POST['type']) && $_POST['type'] == 'brand') {
					...
					$db_query = "SELECT * FROM tbl_products WHERE brand = '$category' ORDER BY title $sort";

                } elseif(isset($_POST['type']) && $_POST['type'] == 'color') {

					$db_query = "SELECT * FROM tbl_products WHERE color = '$category' ORDER BY title $sort";

                } else {
                    $category = (int)$category;
					if($category > 0) {
						$db_query = "SELECT * FROM tbl_products WHERE category = '$category' ORDER BY title $sort";
					} else {
						$db_query = "SELECT * FROM tbl_products ORDER BY title $sort";
					}
                }
...
if($last != 1) {
	if($loadData > 1) {
		$previous = $loadData - 1;
		$paginationlinks .= '<li><a style="cursor: pointer" onclick="loadData_name('.$previous.')"><i class="fa fa-long-arrow-left"></i></a></li>';
		
		//render clikable number links that should appear on the left of the target page number 
		for($i = $loadData-4; $i < $loadData; $i++) {
			if($i > 0) {
				$paginationlinks .= '<li class"active" ><a style="cursor: pointer" onclick="loadData_name('.$i.')">'.$i.'</a></li>';
			}
		}
	}
	...
	if($loadData != $last) {
		$next = $loadData +1;
		$paginationlinks .= '<li><a style="cursor: pointer" onclick="loadData_name('.$next.')"><i class="fa fa-long-arrow-right"></i></a></li>';
	}
}
...
if($sort == 'ASC'){
	$filter = 'A - Z'; ?>
		<input type="hidden" value="ProductA" id="filter_type">
	<?php
} elseif($sort == 'DESC') {
	$filter = 'Z - A'; ?>
		<input type="hidden" value="ProductZ" id="filter_type">
	<?php
}
...
$x=0;	echo("<h3 style='color: #e64567'> $ucategory <i class='fa fa-dot-circle-o animated bounceIn infinite'></i> Filter by Name: $filter</h3>");
?>
                    ...
                    <div class="col-sm-4 col-md-4 col-xs-6">
                        <div class="wa-theme-design-block card">
                            <figure>
                                ...
                                <img src="uploads/images/<?php echo('shop/thumb/product_'.$rsResult['image']); ?>" alt="<?php echo($rsResult['title'].' '.$site_name); ?>">
                                </figure>
                                </a>
                            ...
                            <div class="block-caption1">
                                ...
                                <div class="clear"></div>
                                <h4><a href="product-details/<?php echo($rsResult["url_title"]); ?>"><?php echo($rsResult['title']); ?></a></h4>
...
echo($strhtml);
			...
			<div style='margin-left: 10%'>
				...
				<h4>No Products Available <?php if($category > 0) { echo('in '.$ucategory). ' category'; } ?></h4>
				<input type="hidden" value="<?php echo($category); ?>'" id="searched_cat">
			</div>
		<?php }
		?>
