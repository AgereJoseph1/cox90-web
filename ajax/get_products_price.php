...
include('../functions/functions.php');
                ...
                if(isset($_POST['type']) && $_POST['type'] == 'brand') {
					...
					$db_query = "SELECT * FROM tbl_products WHERE brand = '$category' ORDER BY price $sort";

                } elseif(isset($_POST['type']) && $_POST['type'] == 'color') {

					$db_query = "SELECT * FROM tbl_products WHERE color = '$category' ORDER BY price $sort";

                } else {
                    $category = (int)$category;
					if($category > 0) {
						$db_query = "SELECT * FROM tbl_products WHERE category = '$category' ORDER BY price $sort";
					} else {
						$db_query = "SELECT * FROM tbl_products ORDER BY price $sort";
					}
                }
...
if($last != 1) {
	if($loadData > 1) {
		$previous = $loadData - 1;
		$paginationlinks .= '<li><a style="cursor: pointer" onclick="loadData_sort('.$previous.')"><i class="fa fa-long-arrow-left"></i></a></li>';
		
		//render clikable number links that should appear on the left of the target page number 
		for($i = $loadData-4; $i < $loadData; $i++) {
			if($i > 0) {
				$paginationlinks .= '<li class"active" ><a style="cursor: pointer" onclick="loadData_sort('.$i.')">'.$i.'</a></li>';
			}
		}
	}
	...
	if($loadData != $last) {
		$next = $loadData +1;
		$paginationlinks .= '<li><a style="cursor: pointer" onclick="loadData_sort('.$next.')"><i class="fa fa-long-arrow-right"></i></a></li>';
	}
}
...
if($sort == 'ASC'){
	$filter = 'Lowest to Highest'; ?>
		<input type="hidden" value="Lowest" id="filter_type">
	<?php
} elseif($sort == 'DESC') {
	$filter = 'Highest to Lowest'; ?>
		<input type="hidden" value="Highest" id="filter_type">
	<?php
}
...
$x=0;	echo("<h3 style='color: #e64567'> $ucategory <i class='fa fa-dot-circle-o animated bounceIn infinite'></i> Filter by Price: $filter</h3>");
?>
