...
//sleep(5);
				$db_query = "SELECT * FROM tbl_products ORDER BY title ASC";
...
if($last != 1) {
	if($loadData > 1) {
		$previous = $loadData - 1;
		$paginationlinks .= '<li><a style="cursor: pointer" onclick="load_product('.$previous.')"><i class="fa fa-long-arrow-left"></i></a></li>';
		
		//render clikable number links that should appear on the left of the target page number 
		for($i = $loadData-4; $i < $loadData; $i++) {
			if($i > 0) {
				$paginationlinks .= '<li class"active" ><a style="cursor: pointer" onclick="load_product('.$i.')">'.$i.'</a></li>';
			}
		}
	}
	...
	if($loadData != $last) {
		$next = $loadData +1;
		$paginationlinks .= '<li><a style="cursor: pointer" onclick="load_product('.$next.')"><i class="fa fa-long-arrow-right"></i></a></li>';
	}
}
